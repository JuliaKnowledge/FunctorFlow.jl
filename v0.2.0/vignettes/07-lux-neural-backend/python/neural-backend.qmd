...
Even without PyTorch, every diagram can be executed by binding plain Python or NumPy functions and compiling with `compile_to_callable`.

### Building a KET-style diagram

We create a diagram with a linear projection, a softmax activation, and a second linear layer, then compose them into a pipeline.
...
Weight matrices and biases are ordinary NumPy arrays. Each morphism is a callable `x → y`.
...
Each morphism executes independently on the source object, while `pipeline` chains them: `linear1 → activate → linear2`.
