...
`value` field is `0`, and an artifact is *exact* iff every recorded loss is
zero. The emitter records the claimed/observed loss values (default `0`,
i.e. "claimed exact"); the theorems below derive the categorical
consequences **from** those recorded values via `native_decide`, so a
certificate that records a nonzero obstruction loss will fail to type-check
its exactness/commutativity claims. This is the content the earlier
schema-level `True` placeholders lacked. -/
...
theorem LoweringArtifact.allLossesZero_of_check {a : LoweringArtifact}
  ...
  intro l hl
  exact Nat.eq_of_beq_eq_true (h l hl)
