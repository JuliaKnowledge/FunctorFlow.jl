...
To make the categorical structure operational, we now train a small Lux-backed
encoder/predictor pair. The setup mirrors C-JEPA's masked prediction objective:
the predictor must reconstruct a masked entity's representation from the
unmasked context. We use an MSE surrogate of the joint history+future loss
to keep the vignette self-contained — the categorical content (an obstruction
between the masked-context path and the unmasked-target path) is identical.
...
right-Kan kernel that makes the masked entity predictable from its context.
In the full C-JEPA pipeline this scalar MSE would be replaced by the joint
$\mathcal{L}_\text{history} + \mathcal{L}_\text{future}$ objective with EMA
target encoder, but the optimization geometry is the same: minimization
forces the predictor to factor through the minimal sufficient context.
