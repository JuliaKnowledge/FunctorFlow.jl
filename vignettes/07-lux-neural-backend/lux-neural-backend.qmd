...
property is **trainability**. Let's run a real training loop on a DB square,
where the diagram's obstruction loss is the natural objective: the comparator
penalizes the gap between the two parallel paths `:f` and `:g`.
...
shrinks monotonically, demonstrating that gradients flow correctly through
both the morphism layers and the comparator. This closes audit P1-FF-7 by
showing the Lux backend is end-to-end trainable, not just forward-evaluable.
